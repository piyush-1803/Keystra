...
export default function Dashboard({ stats, liveStats, onNavigate }) {
  const {
    ...
    const todaySess = sess.filter(s => new Date(s.start_time).toISOString().split('T')[0] === today);
    const todayKeys = todaySess.reduce((acc, s) => acc + s.keystroke_count, 0);

    // Find Personal Bests
    const mxSpeed = sess.reduce((max, s) => s.avg_wpm > max ? s.avg_wpm : max, 0);
    const mxVol = Object.values(stats.streaks?.dailyKeys || {}).reduce((max, val) => val > max ? val : max, 0);

    // Group recent sessions (last 3)
    const recSess = [...sess].reverse().slice(0, 3);
    ...
    const dItems = last5Days.map(date => {
    ...
    const maxSessionWpm = Math.max(100, ...todaySess.map(s => s.avg_wpm));
    const getY = (wpm) => {
    ...
    if (todaySess.length > 0) {
      const pointsData = todaySess.map(s => s.avg_wpm);
    ...
    return {
      ...
      todayKeysCount: todayKeys,
      maxSpeed: mxSpeed,
      maxVolume: mxVol,
      recentSessions: recSess,
      dayItems: dItems,
      wavePath: wPath,
      areaPath: aPath,
